...
- (instancetype)initWithImages:(NSArray *)images selectedIndices:(NSIndexSet *)selectedIndices borderColors:(NSArray *)colors {
    if (self = [super init]) {
        ...
        [_images enumerateObjectsUsingBlock:^(UIImage *image, NSUInteger idx, BOOL *stop) {
            ...
            [view.imageView setContentMode:UIViewContentModeScaleAspectFill];
...
- (void)willAnimateRotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration {
    ...
    if ([self isViewLoaded] && self.view.window != nil) {
        ...
        blurImage = [blurImage applyBlurWithRadius:10 tintColor:self.tintColor saturationDeltaFactor:.5 maskImage:nil];
...
- (void)showInViewController:(UIViewController *)controller animated:(BOOL)animated {
    ...
    contentFrame.origin.y = 65.0;
    contentFrame.size.height = self.view.bounds.size.height - 64.0;
    self.contentView.frame = contentFrame;

    
    UILabel *dashboardItemLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.0, 135.0, self.width, 30.0)];
    [dashboardItemLabel setText:@"Dashboard"];
    [dashboardItemLabel setTextAlignment:NSTextAlignmentCenter];
    [dashboardItemLabel setFont:[UIFont mainFontWithSize:20]];
    [dashboardItemLabel setTextColor:[UIColor whiteColor]];
    [self.contentView addSubview:dashboardItemLabel];
    
    UILabel *forecastItemLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.0, 275.0, self.width, 30.0)];
    [forecastItemLabel setText:@"Forecast"];
    [forecastItemLabel setTextAlignment:NSTextAlignmentCenter];
    [forecastItemLabel setFont:[UIFont mainFontWithSize:20]];
    [forecastItemLabel setTextColor:[UIColor whiteColor]];
    [self.contentView addSubview:forecastItemLabel];
    
    UILabel *dailyValuesItemLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.0, 410.0, self.width, 30.0)];
    [dailyValuesItemLabel setText:@"Daily Values"];
    [dailyValuesItemLabel setTextAlignment:NSTextAlignmentCenter];
    [dailyValuesItemLabel setFont:[UIFont mainFontWithSize:20]];
    [dailyValuesItemLabel setTextColor:[UIColor whiteColor]];
    [self.contentView addSubview:dailyValuesItemLabel];
    
    UILabel *microSocialItemLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.0, 545.0, self.width, 30.0)];
    [microSocialItemLabel setText:@"Social"];
    [microSocialItemLabel setTextAlignment:NSTextAlignmentCenter];
    [microSocialItemLabel setFont:[UIFont mainFontWithSize:20]];
    [microSocialItemLabel setTextColor:[UIColor whiteColor]];
    [self.contentView addSubview:microSocialItemLabel];
    
    
    
    UIView *barLine = [[UIView alloc] initWithFrame:CGRectMake(0.0, 44.0, self.contentView.frame.size.width, 1.0)];
    [barLine setBackgroundColor:[UIColor whiteColor]];
    
    CGRect menuLabelFrame = CGRectMake(0.0, 20.0, contentFrame.size.width, 45.0);
    self.menuLabel = [[UILabel alloc] initWithFrame:CGRectMake(-320.0, 20.0, contentFrame.size.width, 44.0)];
    [self.menuLabel addSubview:barLine];
    ...
    [self.view insertSubview:self.menuLabel belowSubview:self.contentView];
    ...
    [self.view insertSubview:self.blurView belowSubview:self.menuLabel];
    ...
    void (^animations1)() = ^{
        //self.menuLabel.frame = menuLabelFrame;
        self.contentView.frame = contentFrame;
        self.blurView.frame = blurFrame;
    };
    void (^completion1)(BOOL) = ^(BOOL finished) {
    ...
    void (^completion2)(BOOL) = ^(BOOL finished) {
        if (finished && [self.delegate respondsToSelector:@selector(sidebar:didShowOnScreenAnimated:)]) {
            [self.delegate sidebar:self didShowOnScreenAnimated:animated];
        }
    };
    ...
    if (animated) {
        ...
        [UIView animateWithDuration:self.animationDuration
                         ...
                         animations:^{
                             self.contentView.frame = contentFrame;
                             self.blurView.frame = blurFrame;
                             self.menuLabel.frame = CGRectMake(-320.0, 20.0, self.contentView.frame.size.width , self.menuLabel.frame.size.height);
                         }
...
- (void)layoutItems {
    ...
    CGFloat topPadding = leftPadding*1.5;
    
    //UILabel *menuItemLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.0, 130.0, self.width, 30.0)];
    //[menuItemLabel setText:@"Dashboard"];
    [self.itemViews enumerateObjectsUsingBlock:^(RNCalloutItemView *view, NSUInteger idx, BOOL *stop) {
